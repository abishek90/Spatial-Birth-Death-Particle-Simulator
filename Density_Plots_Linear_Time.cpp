...
public:
	...
	void run_system(int max_time)
	{
		...
		// This needs to be modified to be able to produce the data.
		int mcmax = 3;
		double max_density = 0.3;
		std::vector<double> avg_density_final;
		avg_density_final.resize(0);

		std::vector<double> local_lambda;
		local_lambda.resize(0);

		int max_data_points = 20;
		for (int dense_count = 1; dense_count <= max_data_points; dense_count++)
			{
				lambda = (double)(dense_count*max_density/max_data_points) ;
				std::vector<double> local_density;
				local_density.resize(0);
				local_lambda.push_back(lambda);


				for(int mcsims = 1; mcsims<= mcmax; mcsims++)
				{
					particles.resize(0);
					rates.resize(0);
					death_probability.resize(0);
					...
					//collect_statistics();
					local_density.push_back(particles.size()/pow(S,2));
				}
				double temp_sum = 0;
				for(int counter_push = 0;counter_push < local_density.size();counter_push++)
				{
					temp_sum += local_density[counter_push];
				}
				avg_density_final.push_back(temp_sum/local_density.size());
				cout<<"Finished Density "<<dense_count<<endl;
				particles.resize(0);
			...
			ofstream MyFile;
			MyFile.open("Density_Plots_1_New.csv",ios::out | ios::ate | ios::app | ios:: binary);
			MyFile<<"New Experiment"<<endl;
			for(int counters = 0;counters<avg_density_final.size(); counters++)
			{
				MyFile<<avg_density_final[counters]<<","<<local_lambda[counters]<<endl;
			}
