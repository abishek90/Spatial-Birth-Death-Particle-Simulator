...
public:
	...
	std::vector<double> lifetime_vector;
	double total_rate;
	double noise ;
	double lambda ;
	int death_count;
	...
	void generate_next_time_step(int timestep)
	{
		...
		double event_time = -log(1 - get_uniform_random_number())/total_event_rate ;
		...
		if(U <= total_birth_rate/total_event_rate)
		{
			...
			for(int lvar = 0; lvar < particles.size()-1; lvar++)
			{
				...
				particles[lvar]->life_time += event_time;
			}
			particles[particles.size()-1]->interference = run_sum_l;
			particles[particles.size()-1]->life_time = 0;
		...
		else
		{
			...
			for(loopvar = 0; loopvar < Nppl; loopvar++)
			{
				...
				if (UU <= cumulative_death_probability[loopvar]) // loopvar is the index killed
				{
					...
					if (timestep > 100000)
						lifetime_vector.push_back(particles[loopvar]->life_time + event_time);
	...
	void run_system(int max_time)
	{
		...
		{
			compute_probability();
			generate_next_time_step(timestep);
			if(timestep%500 == 0)
	...
	void collect_statistics()
	{
		...
		ofstream MyFileDelay;
		MyFileDelay.open("Delays_40.csv",ios::out | ios::ate | ios::app | ios:: binary);
		for(int i =0; i< lifetime_vector.size(); i++)
		{
			MyFileDelay<<lifetime_vector[i]<<endl;
		}
		MyFileDelay.close();
